  ...
  try {
    ...
    if (!batchesError && existingBatches && existingBatches.length > 0) {
      ...
      if (existingBatches.some(batch => batch.questions)) {
        ...
        batches = [];
        for (let i = 0; i < Math.ceil(questions.length / BATCH_SIZE); i++) {
          const batchQuestions = questions.slice(i * BATCH_SIZE, (i + 1) * BATCH_SIZE);
          batches.push({ 
            batchNumber: i + 1, 
    ...
    if (options.processSingleBatch || options.batchId) {
      ...
      const previouslyProcessed = completedBatches * BATCH_SIZE;
